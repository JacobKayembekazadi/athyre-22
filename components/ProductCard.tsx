...
const ProductCard: React.FC<ProductCardProps> = ({ product, onAddToCart }) => {
  ...
  return (
    ...
    >
      {/* Image Container */}
      <div className="relative aspect-[4/5] overflow-hidden bg-gray-100 rounded-lg md:rounded-none">
        <img
        ...
        {/* Quick Add Overlay - Desktop only, mobile uses tap */}
        ...
        {showSizes && (
          <div className="md:hidden absolute inset-x-2 bottom-12 bg-white p-3 rounded-lg shadow-2xl animate-fade-in">
      ...
      <div className="pt-2 md:pt-4 pb-1 md:pb-2">
        <div className="flex justify-between items-start mb-0.5 md:mb-1 gap-2">
          <h3 className="font-semibold text-xs md:text-sm tracking-tight line-clamp-2">{product.title}</h3>
        ...
        <div className="mt-2 md:mt-3 flex gap-1 md:gap-1.5">
          {product.colors.map(color => (
            <div
              key={color}
              title={color}
              className={`w-2.5 h-2.5 md:w-3 md:h-3 rounded-full border border-gray-100 cursor-pointer ${
